...
class task_time_control_confirm_wizard(osv.osv_memory):
    ...
    def close_confirm(self,cr,uid,ids,context={}):
        ...
        if other_users_in_task == 0:
            ttype = self.pool.get('project.task').stage_find(cr, uid, [started_task], False, [('state', '=', 'open')], context=context)
        ...
        if wizard.task_to_start.id != False:
            ...
            if wizard.task_to_start.state == 'draft':
                wizard.task_to_start.do_open()
            ttype = self.pool.get('project.task').stage_find(cr, uid, [started_task], False, [('state', '=', 'working')], context=context)
